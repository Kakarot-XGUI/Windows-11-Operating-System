...
window.addEventListener("load", () => {
  ...
  // Step 1: Hide boot texts and show logo
  setTimeout(() => {
  ...
  // Step 4: Fade out and remove loader after animations
  setTimeout(() => {
    winLoader.style.opacity = "0";
    winLoader.style.pointerEvents = "none";

    setTimeout(() => {
      winLoader.remove(); // remove from DOM
    }, 500); // matches the fade-out transition
  }, 2500); // total delay before removal
});
...
function addTaskbarIcon1() {
  ...
  const icon = document.createElement("div");
  icon.id = "Pc-icon";
  icon.className = "bg-transparent  top-3 py-2  w-[5vh] rounded cursor-pointer";
